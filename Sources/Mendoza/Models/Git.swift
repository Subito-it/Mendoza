...
struct Git {
    ...
    func clean() -> Bool {
        (try? executer.capture("git diff --exit-code"))?.status == 0
    }

    func status() throws -> GitStatus {
        GitStatus(url: try url(), branch: try branchName(), commitMessage: try commitMessage(), commitHash: try commitHash())
    }
    ...
    func branchName() throws -> String {
        try executer.execute(#"git show -s --pretty=%D HEAD | sed -e 's/ -> / /g' | sed -e 's/,//g' | sed -e 's/origin\///g' | awk '{print $2}'"#)
    }

    func commitMessage() throws -> String {
        try executer.execute("git log -1 --pretty=%B HEAD")
    }

    func commitHash() throws -> String {
        try executer.execute("git rev-parse HEAD")
    }
}
