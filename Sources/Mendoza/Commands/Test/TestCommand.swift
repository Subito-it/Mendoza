...
class TestCommand: Command {
    ...
    let deviceName = Argument<String>(name: "name", kind: .named(short: "d", long: "device_name"), optional: true, help: "Device name to use to run tests. e.g. 'iPhone 8'")
    ...
    func run() -> Bool {
        do {
            let device: Device
            if let deviceName = deviceName.value, let deviceRuntime = deviceRuntime.value {
