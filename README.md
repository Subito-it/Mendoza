...
`--test_batch_size` accepts any positive number and defaults to `1`. Selected tests run sequentially in one `xcodebuild test-without-building` invocation per runner, while runners continue operating concurrently. The last batch may contain fewer tests, and failed-test retries always run individually. The same node-local executor, watchdog, and coverage pipeline are used on iOS simulators and macOS.
...
Test verdicts, durations, progress and retry limits remain per test. Xcode may execute selected tests in a different order. An interrupted batch preserves completed verdicts, fails the active test, and returns every unstarted test to the queue as a singleton without consuming a retry. A launch failure consumes one failed attempt; the remaining tests are returned. Test plans that repeat tests are unsupported.

Coverage is extracted once after the invocation completes. Both `--individual_test_coverage` and `--test_covered_files` produce **combined batch coverage**: a passing test's report can include code executed by other batch members, including failing members. Passing members receive the existing `Suite-test-timestamp.json` filenames containing the same combined report. Canonical reports live under `batches/<invocation UUID>/coverage.json` inside each coverage output directory, alongside `metadata.json` describing membership and possible incomplete coverage. Unstarted tests receive no report. Each batch enters aggregate session coverage exactly once. Missing coverage for a started batch, failed merges, and failed requested exports fail the session explicitly; valid partial coverage from interrupted tests is marked in metadata.

With `--skip_result_merge`, every member references the same xcresult. A batch owns one result bundle and transfers it once. Raw batch output and invocation metadata are collected into `results/batch_logs/<invocation UUID>`.

`--stdout_timeout` applies only while a test is active and resets for each test. Separate limits bound startup (180 seconds), the gap between tests (120 seconds), and result finalization (900 seconds). On interruption Mendoza sends INT to cancel the invocation and, for iOS, terminates the apps on that simulator. It then escalates to TERM after 10 seconds and KILL after 15 seconds if necessary. Signals target the invocation's own process group. Other runners' xcodebuild processes are unaffected.
